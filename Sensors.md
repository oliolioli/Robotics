...
We output the two constants PROX LEFT FRONT and PROX RIGHT FRONT using a print command and determine that the numbering of the sensors (7, 0) matches the specification.
The robot is now positioned in an obstacle-free environment so that the calibration is not disturbed by shadows and the like. After starting this script, we observe the robot and check its calibration. The calibration is signalled by LEDs. As soon as this is finished, we place an obstacle
obstacle directly in front of the robot and initialise the distance measurement.
